-- 0015_repeater_delay.sql — The repeater record learns its delay (repeater
-- backbone paths).
--
-- A repeater waits before re-transmitting a message its pass ranges accept, so
-- that two repeaters which both heard it don't answer in the same instant and
-- collide at every receiver that hears both. That wait is a per-repeater
-- setting in the field hardware; this file gives the station record a place to
-- hold it: meganet.repeater.delay_ms, integer milliseconds, checked under
-- 1000. Null means "not set" — the app computes a *suggested* value from the
-- backbone adjacency (see map-backbone.js) but never writes it silently, the
-- TBRGbucketSize convention: a recorded value is knowledge, a suggestion is
-- labelled as the guess it is.
--
-- The document carries it conditionally, like every other nullable — absent
-- until somebody records a value — so the committed stations.json is
-- byte-stable until then. save_station() round-trips it, and
-- load_stations_doc() reads it back in (0013's lesson: the loader is the half
-- that gets forgotten, so it is restated here with the field rather than dug
-- up as a round-trip hole later).
--
-- Forward-only, idempotent, no begin/commit — see db/README.md:
--
--   psql "$MEGANET_DB_URL" -v ON_ERROR_STOP=1 --single-transaction \
--        -f db/migrations/0015_repeater_delay.sql

-- ── The column ───────────────────────────────────────────────────────────────

alter table meganet.repeater
  add column if not exists delay_ms integer
    check (delay_ms >= 0 and delay_ms < 1000);

comment on column meganet.repeater.delay_ms is
  'How long this repeater waits before re-transmitting a message its pass ranges accept, in milliseconds (< 1000). Null = not set; the app suggests a collision-avoiding value from the backbone adjacency but never records one silently. stations.json stations[].repeater.delay_ms.';

-- ── The document: one more conditional key ───────────────────────────────────
-- Restated from 0013 with delay_ms appended to the repeater fragment when set.
-- stations_json (the whole document) selects from this view, so it follows
...
-- ── Saving ───────────────────────────────────────────────────────────────────
-- Restated from 0013 with the early range validation (a friendly answer beats
-- a check-constraint name), delay_ms in the repeater insert list, and — the
-- half 0011's lesson says is the one that actually gets forgotten — in the
-- on-conflict update set, so the value survives a second save.
...
-- ── Loading ──────────────────────────────────────────────────────────────────
-- Restated from 0013 with delay_ms in the repeater insert, update and
-- comparison lists. The loader treats the document as authoritative, so a
-- document that omits the value clears it — the contract every other column
-- has.
...
begin
  if not exists (
    select 1 from information_schema.columns
     where table_schema = 'meganet' and table_name = 'repeater'
       and column_name = 'delay_ms') then
    raise exception '0015 did not take: meganet.repeater.delay_ms is missing';
  end if;
end
$$;

-- ── Schema version ───────────────────────────────────────────────────────────
-- DB_SCHEMA_VERSION in core.js goes 14 → 15 in the same commit as this file.
