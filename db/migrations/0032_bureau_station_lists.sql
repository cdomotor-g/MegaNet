-- 0032_bureau_station_lists.sql — The rest of the Bureau's Queensland flood
-- warning station lists: which of its indexes list a station, the station's
-- AWRC number, stream and URBS label, and what each height on its gauge means.
--
-- Why this file exists
-- ────────────────────
-- 0031 took four sections of the Bureau's lists — flood classifications,
-- crossings, survey details — and attached them to the half of the stations
-- they describe that MegaNet had. Five more documents are now in
-- archive/river-height-stations/, and between them they name every station in
-- the service and say where it is:
--
--   Section 1, 25/09/2026   index of FloodWarn rainfall stations  ┐ number, name,
--   Section 2, 26/09/2026   index of daily rainfall stations      │ basin, position;
--   Section 3, 26/09/2026   index of river height stations        ┘ Section 3 adds
--                                                                   the AWRC number
--                                                                   and the stream
--   Section 9, 26/09/2026   flood effects — what each height on a gauge means on
--                           the ground: "Minor Flood Level", "Bridge (Pacific
--                           Highway Bridge)", "Low lying roads at Budds Beach"
--   URBS details, 26/09/2026  the label each river height station goes by in
--                           the Bureau's URBS runoff-routing model
--
-- This file makes somewhere for what they add:
--
--   meganet.station.awrc_number   Section 3's AWRC number: the gauging station
--                                 number of the national (Australian Water
--                                 Resources Council) scheme, whose first three
--                                 digits are the basin — 136912 is in the
--                                 Burnett, basin 136. Text, because it is an
--                                 identifier with leading zeros (011902).
--   meganet.station.stream        the stream the gauge is on, as Section 3
--                                 prints it.
--   meganet.station.urbs_label    the station's node in the URBS model, as the
--                                 URBS details print it. Stations share one
--                                 when they read the same place — a TM and the
--                                 ALERT gauge beside it — so it is not unique.
--   meganet.station_bureau_listing  one row per index a station is listed in,
--                                 dated: which of the Bureau's services it is
--                                 part of, and as at when. A list, like 0031's,
--                                 because the next edition of an index is a
--                                 new row beside this one, not an overwrite.
--   meganet.station_flood_effect  Section 9's rows: a height on the gauge, the
--                                 effect the Bureau writes against it, and the
--                                 detail the page prints under some of them in
--                                 brackets — "Bridge" over "(Pacific Highway
--                                 Bridge)" is effect and detail.
--
-- and `meganet.bureau_index`, the three indexes, for the listing's section to
-- be a foreign key into — a section nobody knows is refused rather than kept.
--
-- The URBS details also print each station's minor, moderate and major
-- levels. On all 921 rows that print them they equal Section 4's, which 0031
-- already stores, so they are not stored twice.
--
-- What this file does NOT do
-- ──────────────────────────
-- It carries no station data, for 0029's reason. The stations the indexes list
-- and MegaNet does not have — 1,697 of them, a field station each, as the
-- editor's "+ New" makes — and every row of every list are written by
--
--   python3 tools/ingest/river_height_stations.py --sql \
--     | psql "$MEGANET_DB_URL" -v ON_ERROR_STOP=1 --single-transaction
--
-- which is additive by construction: a station is created only where no
-- station, live or deleted, has its number; a list is attached only where the
-- station has none of that edition yet; the three fields are set only where
-- they are empty; and nothing already on a station — its position above all —
-- is written over. tools/ingest/river_height_stations.py says why at length.
--
-- What had to be restated
-- ───────────────────────
-- The same three objects 0031 restated, verbatim from 0031 with the two lists
-- and three fields threaded through: station_json (two more optional lists and
-- three optional keys, each absent where the station has none), save_station()
-- and load_stations_doc().
--
-- save_station() treats the two lists as it treats 0031's: absent from the
-- document means leave alone, present — empty included — means replace. The
-- three fields are ordinary optional keys, like `owner`: the editor starts its
-- document from the record it loaded, so a key it does not show is carried
-- through untouched, and a blank one is stored as null.
--
-- No grants are repeated with the restatements; 0029 says why.
--
-- Idempotent and forward-only, per db/README.md.

-- ── The station's three fields ───────────────────────────────────────────────

alter table meganet.station add column if not exists awrc_number text;
alter table meganet.station add column if not exists stream text;
alter table meganet.station add column if not exists urbs_label text;

comment on column meganet.station.awrc_number is
  'The AWRC gauging station number, as Section 3 of the Bureau''s Queensland flood warning station lists prints it (136912). Its first three digits are the AWRC basin. Null means not recorded.';
comment on column meganet.station.stream is
  'The stream the station''s gauge is on, as Section 3 of the Bureau''s lists prints it. Null means not recorded.';
comment on column meganet.station.urbs_label is
  'The station''s node label in the Bureau''s URBS runoff-routing model, as the URBS details print it. Not unique: a TM and the ALERT gauge beside it read the same place. Null means not recorded.';

-- ── The vocabulary ───────────────────────────────────────────────────────────
-- The Bureau's own numbering is the code: "Section 3" is what the pages say and
-- what anybody holding one would look for.

create table if not exists meganet.bureau_index (
...
comment on table meganet.bureau_index is
  'The indexes of the Bureau''s Queensland flood warning station lists — Sections 1, 2 and 3 — by the section number the pages print.';

insert into meganet.bureau_index (code, label, title, ord) values
  ('1', 'FloodWarn rainfall', 'Index of Queensland FloodWarn rainfall stations',       0),
  ('2', 'Daily rainfall',     'Index of Queensland daily reporting rainfall stations', 1),
  ('3', 'River height',       'Index of Queensland river height stations',             2)
on conflict (code) do update
   set label = excluded.label, title = excluded.title, ord = excluded.ord
 where (meganet.bureau_index.label, meganet.bureau_index.title, meganet.bureau_index.ord)
       is distinct from (excluded.label, excluded.title, excluded.ord);

-- ── The two lists ────────────────────────────────────────────────────────────
-- 0031's shape: hung off the station, `on delete cascade`, keyed by
-- (station_id, ord), a blank row refused by a check and dropped by both write
-- paths before it gets there.

create table if not exists meganet.station_bureau_listing (
  station_id  text        not null references meganet.station (id) on delete cascade,
  ord         integer     not null,
  section     text        not null references meganet.bureau_index (code),
  -- The date on the index this came from, or, for a row an editor adds, the
  -- date the station joined the service.
  as_at       date,
  note        text,
  updated_at  timestamptz not null default now(),
  updated_by  text,
...
comment on table meganet.station_bureau_listing is
  'Which of the Bureau''s Queensland flood warning station indexes (Sections 1–3) list a station, and as at when: one row per index per edition.';

create table if not exists meganet.station_flood_effect (
  station_id  text        not null references meganet.station (id) on delete cascade,
  ord         integer     not null,
  as_at       date,
  height_m    numeric,
  ...
  updated_by  text,
  primary key (station_id, ord),
  constraint station_flood_effect_carries_something check (
    num_nonnulls(height_m, effect, detail, note) > 0)
);

comment on table meganet.station_flood_effect is
  'What each height on a station''s gauge means on the ground, as Section 9 of the Bureau''s Queensland flood warning station lists prints it: the height in metres on the gauge, the probable flood effect, and the detail printed under it in brackets. One row per height per edition, in the page''s order.';
comment on column meganet.station_flood_effect.detail is
  'The line the page prints in brackets under an effect — which bridge, which spillway — without its brackets.';

do $$
declare
  t text;
begin
  foreach t in array array['bureau_index', 'station_bureau_listing', 'station_flood_effect']
  loop
...
-- ── Row level security ───────────────────────────────────────────────────────
-- 0031's, for the same reasons: read for everyone, written through
-- save_station() and the loaders, the editor policies as belt and braces, and
-- none at all on the vocabulary.
...
begin
  ...
  foreach t in array array['station_bureau_listing', 'station_flood_effect']
  loop
...
begin
  ...
  grant select on meganet.bureau_index, meganet.station_bureau_listing,
                  meganet.station_flood_effect
    to anon, authenticated;

  grant select, insert, update, delete
     on meganet.bureau_index, meganet.station_bureau_listing,
        meganet.station_flood_effect
    to service_role;
...
-- ── meganet.station_json — 0031's view, plus the two lists and three fields ──
...
-- ── meganet.save_station() — 0031's function, plus the two lists and three fields
-- The editor's write path. Everything 0031 did, unchanged, and then 0032's lists
-- when the document carries them and its fields always.
...
begin
  ...
  foreach v_key in array array['flood_classes', 'crossings', 'gauge_survey',
                               'bureau_listings', 'flood_effects'] loop
...
-- ── meganet.load_stations_doc() — 0031's function, plus the two lists and three
-- fields. The importer's write path, syncing them the way it syncs 0031's.
...
begin
  if not exists (select 1 from pg_catalog.pg_views
                  where schemaname = 'meganet' and viewname = 'station_json'
                    and definition like '%''bureau_listings''%'
                    and definition like '%''flood_effects''%'
                    and definition like '%''awrc_number''%'
                    and definition like '%''urbs_label''%'
                    and definition like '%''gauge_survey''%') then
    raise exception '0032 did not take: station_json does not emit the two lists and three fields';
  end if;
  if (select count(*) from pg_catalog.pg_proc p
        join pg_catalog.pg_namespace n on n.oid = p.pronamespace
       where n.nspname = 'meganet' and p.proname = 'save_station'
         and p.prosrc like '%station_bureau_listing%'
         and p.prosrc like '%station_flood_effect%'
         and p.prosrc like '%excluded.urbs_label%'
         and p.prosrc like '%station_gauge_survey%') = 0 then
    raise exception '0032 did not take: save_station does not write the two lists and three fields';
  end if;
  if (select count(*) from pg_catalog.pg_proc p
        join pg_catalog.pg_namespace n on n.oid = p.pronamespace
       where n.nspname = 'meganet' and p.proname = 'load_stations_doc'
         ...
         and p.prosrc like '%excluded.awrc_number%'
         and p.prosrc like '%excluded.catchment_area_km2%') = 0 then
    raise exception '0032 did not take: load_stations_doc does not sync the two lists and three fields';
  end if;
  if (select count(*) from meganet.bureau_index) < 3 then
    raise exception '0032 did not take: the Bureau''s three indexes are not all there';
  end if;
end
$$;

-- ── Schema version ───────────────────────────────────────────────────────────
-- DB_SCHEMA_VERSION in core.js goes 31 → 32 in the same commit as this file.
