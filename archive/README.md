...
| `aep-levels/QLD_AEP_Levels_1.xlsx`, `aep-levels/NSW_AEP_Levels_1.xlsx` | Modelled water levels at flood warning stations in the 1%, 0.5%, 0.2% and 0.066% AEP floods, with the ground height and the sheet's own confidence scores, supplied 26/09/2026. `tools/ingest/aep_levels.py` reads them into `../data/aep-levels.json` and the station records (`db/migrations/0033_aep_levels_and_frequencies.sql`), where the station card shows them as indicative and works an indicative flood velocity out of them. Read by header name and checked word for word, so keep them as they are. |
| `QLD_SLS_current.pdf` | The Bureau's *Service Level Specification for Flood Forecasting and Warning Services for Queensland*, version 3.7 (December 2025), as the Bureau publishes it at <https://www.bom.gov.au/qld/flood/brochures/QLD_SLS_current.pdf> — which is what the heading of the station card's SLS section links to. `tools/ingest/sls.py` reads six of its schedules into `../data/sls-qld.json` and `../data/sls-locations.json` (`db/migrations/0028_sls_schedules.sql`). |
| `QLD_SLS_v3.1_2018-09.pdf` | Version 3.1 (September 2018), which those files were read from until 3.7. Kept for comparison; the same tool still reads it with `--pdf`. |
